...
import (
	...
	"github.com/dmmartynyuk/neuromerch/rbfnet"
)
...
func calcnet(kStore string, kGoods string) (retNext int, retDemand float64) {
	...
	if islog {
		err := dbm.DbLog("beg. начало составления прогноза", "calculate", Mc.Val())
		if err != nil {
	...
	defer func() {
		if r := recover(); r != nil {
			dbm.DbLog("err. конец составления прогноза по recover "+r.(error).Error(), "calculate", time.Now().UTC().UnixNano())
			log.Printf("err. конец составления прогноза по recover")
		}
		if islog {
			dbm.DbLog("end. конец составления прогноза", "calculate", time.Now().UTC().UnixNano())
		}
	}()
	defer Mc.Set(0)
	...
	//contracts, err = dbm.GetContracts()

	stores, err := dbm.GetMagNames(1, kStore)

	if err != nil {
		dbm.DbLog("calc. Ошибка чтения склада "+err.Error(), "calculate", time.Now().UTC().UnixNano())
		return
	...
	for _, store := range stores {
		...
		var sales = new(dbm.Sales)
		var goods []dbm.MatrixGoods
		tipSales := "S"
		if store.Tip < 1 { //для распределительного склада не считаем, там суммовое
			continue
		}
		if store.Tip > 1 {
			goods, err = dbm.GetAllGoodsFromMatrix(uidstore, kGoods)
		} else { //==1
			goods, err = dbm.GetOptMatrix(uidstore, kGoods, MAXSALES)
			//для оптового склада и продажи, и движения
			tipSales = "SM"
		}
		if err != nil {
			dbm.DbLog("calc. Ошибка чтения  матрицы магазина "+store.Name+" "+err.Error(), "calculate", time.Now().UTC().UnixNano())
			return
		...
		for _, merch := range goods {
			if Mc.Val() < 0 {
				//надо завершиться
				Mc.Set(0)
				dbm.DbLog("stop. завершено по сигналу", "calculate", time.Now().UTC().UnixNano())
			...
			*/
			predict[0] = 0.0
			predict[1] = 0.0
			sales, err = dbm.GetSales(uidstore, merch.KeyGoods, time.Now().AddDate(0, 0, -MAXSALES).Format("2006-01-02"), time.Now().Format("2006-01-02"), tipSales)
			cnt := len(sales.Cnt)
			if err != nil {
				dbm.DbLog("calc. Ошибка чтения продаж магазина "+store.Name+" "+err.Error(), "calculate", time.Now().UTC().UnixNano())
				return
			...
			gt, err := dbm.GetGood(merch.KeyGoods)
			if err != nil {
				dbm.DbLog("calc. Ошибка чтения товара "+merch.KeyGoods+" "+err.Error(), "calculate", time.Now().UTC().UnixNano())
				return
			...
			if cnt < KC*2 {
				...
				if now-lastdeal < 45 {
					...
					retDemand = d
					dbm.SavePredict(uidstore, merch.KeyGoods, meancnt, now, int(sigmapd), d)
			...
			dbm.SaveRbfNet(uidstore, merch.KeyGoods, string(strrbf), sales.LastPeriod, float64(pd))
			dbm.SavePredict(uidstore, merch.KeyGoods, p1, now, pd, predict[0])
...
func apiMakeOrders(uidstorearg, uidgoodarg string) {
	//var stores dbm.Stores
	var err error
	var now = time.Now() //сегодня
	var provider string

	Fop.Set(time.Now().UTC().UnixNano())
	err = dbm.DbLog("beg makeOrders. Начало составления заказов", "makeOrders", Fop.Val())
	if err != nil {
	...
	defer func() {
		if r := recover(); r != nil {
			dbm.DbLog("err. конец составления заказов по recover "+r.(error).Error(), "makeOrders", time.Now().UTC().UnixNano())
			log.Printf("err. конец составления заказов по recover")
		}
		dbm.DbLog("end. конец составления заказов", "makeOrders", time.Now().UTC().UnixNano())
	}()
	defer Fop.Set(0)
	...
	numzak := dbm.GetLastNumZakaz(now.Format("2006-01-02"))
	//читаем все контракты

	delivdays := 1
	//на основе данных по доставке делаем заказ
	var contracts []dbm.Contract
	contracts, err = dbm.GetContracts(uidstorearg)
	if err != nil {
		//несмогли прочитать контракты, ругаемся
		dbm.DbLog("makeOrders. Ошибка чтения контрактов "+err.Error(), "makeOrders", time.Now().UTC().UnixNano())
		return
	}
	for _, contract := range contracts {
		inched := true
		if strings.Contains(contract.Chedord, "/") {
			t, err := dbm.GetLastOrd(contract.Provider)
			if err == nil {
		...
		if inched {
			...
			stores, err := dbm.GetMagNames(0, contract.Recipient)
			if len(stores) == 0 {
				continue
			}
			store := stores[0]
			...
			isstore, err := dbm.GetMagNames(0, provider)
			//var provgoods map[string]dbm.Goods
			var outlineprovider bool = false
			if len(isstore) == 0 {
				//внешний поставщик
				outlineprovider = true
				delivdays = contract.Delivdays
				//provgoods, err = dbm.GetProviderGoods(provider, uidgoodarg)
			}
			...
			case store.Tip >= 2:
				...
				goods, err = dbm.GetAllGoodsFromMatrix(uidstore, uidgoodarg)
			case store.Tip == 1:
				goods, err = dbm.GetOptMatrix(uidstore, uidgoodarg, MAXSALES)
			case store.Tip == 0:
				//соберет все остатки, кроме складов, которых нет в матрице
				goods, err = dbm.GetCenterMatrix(uidgoodarg, provider)
			}

			if err != nil {
				dbm.DbLog("makeOrders. Ошибка чтения  матрицы магазина "+store.Name+" "+err.Error(), "makeOrders", time.Now().UTC().UnixNano())
				return
			}
			...
			for _, merch := range goods {
				...
				if Fop.Val() < 0 {
					//надо завершиться
					Fop.Set(0)
					dbm.DbLog("stop. завершено по сигналу", "makeOrders", time.Now().UTC().UnixNano())
					log.Printf("stop. завершено по сигналу")
					return
				}
				...
				//уже собрана в goods = dbm.GetCenterMatrix
				...
				if !outlineprovider {
					...
					salestat, _ := dbm.GetSaleStat(uidstore, merch.KeyGoods, DAYSNOSALE)
					...
					_, lb, _ := dbm.GetLastBalance("", merch.KeyGoods)
					if lb > 0 {
						merch.Balance = lb
					}
					//if merch.PredDemand <= 0 || merch.PredDemand > 1 {
					salestat, _ := dbm.GetSaleStat("", merch.KeyGoods, DAYSNOSALE)
				...
				if !outlineprovider {
					_, provbalance, _ := dbm.GetLastBalance(provider, merch.KeyGoods)
					if provbalance == 0 {
						analog, ost, _ := dbm.GetAnalog(provider, merch.KeyGoods)
						if len(analog) > 0 {
							var gds *dbm.Goods
							if ost > 0 {
								gds, _ = dbm.GetGood(merch.KeyGoods)
							} else {
								gds, _ = dbm.GetGood(analog)
							}
							gname := gds.Name
							if len(gname) > 0 {
				...
				if cntzak > 0.0 && (int)(cntzak+round) > 0 {
					dbm.SaveOper(ordersnum, provider, uidstore, merch.KeyGoods, now.Format("2006-01-02"), float64((int)(cntzak+round)), next.Format("2006-01-02"), datedelivdays, explain)
				}
				...
				dbm.UpdateMatrix(m, w)
			} //по товарам
		} /*else { //если стоит в расписании
			//если в расписании нет, то смотрим позиции, котоые были из заказов удалены
			stores, _ := dbm.GetMagNames(0, contract.Provider)
			if len(stores) == 0 {
				continue
			}
			provider := stores[0]
			if provider.Tip == 0 { //распределительный склад
				st, _ := dbm.GetReOrdering(contract.Provider, contract.Recipient, time.Now().Format("2006-01-02"))
				for _, uidgoods := range st {
					...
					goods, err = dbm.GetAllGoodsFromMatrix(contract.Recipient, uidgoods)
				}
			}
	...
	dbm.DbLog("end makeOrders. Конец составления заказов", "makeOrders", time.Now().UTC().UnixNano())
}
...
func apiMakeOrdersOpt() {
	...
	Fop.Set(time.Now().UTC().UnixNano())
	err = dbm.DbLog("beg makeOrders. Начало составления заказов для оптовых складов", "makeOrders", Fop.Val())

	conf, err := dbm.GetConfig()
	MINDAYSORD := conf.ValInt("minDaysOrd", 7)
	MAXSALES := conf.ValInt("maxsales", 720)
	//только оптовые склады, тип +100=0
	stores, err := dbm.GetMagNames(-100, "")

	if err != nil {
		dbm.DbLog("makeOrders. Ошибка чтения склада "+err.Error(), "makeOrders", time.Now().UTC().UnixNano())
		return
	}
	numzak := 0
	...
	for _, store := range stores {
		...
		var goods []dbm.MatrixGoods
		//на основе данных по доставке делаем заказ
		var contr []dbm.Contract
		contr, err = dbm.GetContracts(uidstore)
		if err != nil {
		...
		if store.Tip > 0 {
			goods, err = dbm.GetAllGoodsFromMatrix(uidstore, "")
		} else {
			goods, err = dbm.GetOptMatrix(uidstore, "", MAXSALES)
		}
		if err != nil {
			dbm.DbLog("makeOrders. Ошибка чтения  матрицы магазина "+store.Name+" "+err.Error(), "makeOrders", time.Now().UTC().UnixNano())
			return
		...
		for _, merch := range goods {
			...
			//	lp, err := dbm.GetLastPredict(uidstore, merch.KeyGoods)
			//	if err != nil {
			//		dbm.DbLog("makeOrders. Ошибка чтения таблицы предсказаний "+err.Error(), "makeOrders", time.Now().UTC().UnixNano())
			//		return
			//	}
			//если товар заказан уже то не заказываем
			//это условие проверим при записи в заказ
			...
			if cntzak > 0.0 && (int)(cntzak+0.5) > 0 {
				dbm.SaveOper(ordersnum, provider, uidstore, merch.KeyGoods, now.Format("2006-01-02"), cntzak, next.Format("2006-01-02"), datedelivdays)
			}
			//обновим ср потребность в матрице
			var m map[string]interface{}
			m = map[string]interface{}{}
...
func apiRecalcABC(uidStore string, period1 string, period2 string) error {
	lp, sum := dbm.GetProfit(uidStore, period1, period2)
...
func needPredict(uidStore string, uidGoods string) bool {
	...
	nr, err := dbm.GetLastPredict(uidStore, uidGoods)
	if err != nil {
		return true
	}
	//если делался почти
...
func RecalcProfit(keystore, pfrom, pto string) (map[string][]dbm.ProfitGraph, float64, error) {
	...
	stat, err := dbm.GetProfitMounth(keystore, pfrom, pto)
	if err != nil {
		return stat, 1, err
	}
	retqerr := float64(0)
	...
	for uidstore, magstat := range stat {
		...
		for k := 0; k < 3; k++ {
			...
			stat[uidstore] = append(stat[uidstore], dbm.ProfitGraph{})
			if m > 9 {
