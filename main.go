...
import (
	...
	"github.com/dmmartynyuk/neuromerch/dbm"
...
func calculate(c *gin.Context) {
	...
	if start == "ok" {
		/*
			slog := dbm.GetLastStateNetwork(1, "calculate")
			for _, v := range slog {
				if v[:3] != "end" {
	...
	slog := dbm.GetLastStateNetwork(3, "calculate")
...
func fetchAllStocks(c *gin.Context) {
	...
	rows, _, data, err := dbm.GetTable("stores", pg-1, gate, cond)
	if err != nil && rows > 0 {
...
func updateStocks(c *gin.Context) {
	...
	m[0] = matr
	err = dbm.UpdateTableData("stores", m, cond)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
		return
	}
...
func fetchAllContracts(c *gin.Context) {
	...
	if len(providername) > 0 {
		...
		cond = cond + "c.providername='" + dbm.Escape(providername) + "'"
	}
	if len(chedord) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
		}
		cond = cond + "c.chedord='" + dbm.Escape(chedord) + "'"
	}
	if len(delivdays) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
	...
	rows, _, data, err := dbm.GetTable("contracts", pg-1, gate, cond)
	if err != nil && rows > 0 {
		c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
		return
	}
...
func updateContracts(c *gin.Context) {
	...
	m := make([]map[string]interface{}, 1)
	m[0] = matr
	err = dbm.UpdateTableData("contracts", m, cond)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
		return
	}
...
func updateContractGoods(c *gin.Context) {
	...
	*/
	...
	dbm.DbLog("beg. начало обновления товаров в базу "+time.Now().Format("2006-01-02T15:04:05"), "updateContractGoods", time.Now().UTC().UnixNano())
	err := dbm.InsertTableData("contractgoods", matr, w)
	if err != nil {
		dbm.DbLog("err. ошибка обновления товаров в базу "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "updateContractGoods", time.Now().UTC().UnixNano())
		c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		return
	}
	dbm.DbLog("end. конец обновления товаров в базу "+time.Now().Format("2006-01-02T15:04:05"), "updateContractGoods", time.Now().UTC().UnixNano())
...
func fetchAllSalesmatrix(c *gin.Context) {
	...
	if len(storename) > 0 {
		...
		cond = cond + "st.name='" + dbm.Escape(storename) + "'"
	}
	if len(goodsname) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
		}
		cond = cond + "g.name='" + dbm.Escape(goodsname) + "'"
	}
	if len(goodsgroup) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
		}
		cond = cond + "g.groupname='" + dbm.Escape(goodsgroup) + "'"
	}
	if len(art) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
		}
		cond = cond + "g.Art='" + dbm.Escape(art) + "'"
	}
	if len(abc) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
		}
		cond = cond + "s.abc='" + dbm.Escape(abc) + "'"
	}
	if len(inuse) > 0 {
		if len(cond) > 0 {
			cond = cond + " and "
	...
	rows, _, data, err := dbm.GetTable("salesmatrix", pg-1, gate, cond)
	if err != nil && rows > 0 {
		c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
		return
	}
...
func updateSalesmatrix(c *gin.Context) {
	...
	m := make([]map[string]interface{}, 1)
	m[0] = matr
	err = dbm.UpdateTableData("salesmatrix", m, cond)
	if err != nil {
...
func updateGoods(c *gin.Context) {
	...
	*/
	...
	w := make(map[string]string)
	dbm.DbLog("beg. начало обновления товаров в базу "+time.Now().Format("2006-01-02T15:04:05"), "updateGoods", time.Now().UTC().UnixNano())
	err := dbm.InsertTableData("goods", matr, w)
	if err != nil {
		dbm.DbLog("err. ошибка обновления товаров в базу "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "updateGoods", time.Now().UTC().UnixNano())
		c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		return
	}
	dbm.DbLog("end. конец обновления товаров в базу "+time.Now().Format("2006-01-02T15:04:05"), "updateGoods", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
}
...
func updateAnalog(c *gin.Context) {
	...
	*/
	...
	w := make(map[string]string)
	dbm.DbLog("beg. начало обновления аналогов в базу "+time.Now().Format("2006-01-02T15:04:05"), "updateAnalog", time.Now().UTC().UnixNano())
	dbm.DeleteTableData("goodsanalog", del)
	err := dbm.InsertTableData("goodsanalog", matr, w)
	if err != nil {
		dbm.DbLog("err. ошибка обновления аналогов в базу "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "updateAnalog", time.Now().UTC().UnixNano())
		c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		return
	}
	dbm.DbLog("end. конец обновления аналогов в базу "+time.Now().Format("2006-01-02T15:04:05"), "updateAnalog", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
}
...
func fetchSingleGoods(c *gin.Context) {
	...
	if len(goodsuid) > 0 {
		gd, err := dbm.GetGood(goodsuid)
		if err != nil {
			c.JSON(http.StatusOK, gin.H{"status": "err", "message": err.Error()})
			return
		}
		gds := make([]dbm.Goods, 1)
		gds[0] = *gd
...
func getNeuroData(c *gin.Context) {
	...
	nr, err := dbm.LoadRbfNet(storeuid, goodsuid)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"status": "err", "message": err.Error()})
		return
	}
...
func getPredict(c *gin.Context) {
	goodsuid := c.Param("goods")
	storeuid := c.Param("store")
	nr, err := dbm.GetLastPredict(storeuid, goodsuid)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"status": "err", "message": err.Error()})
		return
	}
...
func setSales(c *gin.Context) {
	...
	*/
	...
	w["period"] = "="
	w["tipmov"] = "="
	dbm.DbLog("beg. начало записи движений в базу "+lastuidstore+" записей: "+strconv.FormatInt(int64(len(matr)), 10)+" "+time.Now().Format("2006-01-02T15:04:05"), "setSales", time.Now().UTC().UnixNano())
	err = dbm.InsRepSales(matr, w)
	if err != nil {
		dbm.DbLog("err. ошибка записи движений в базу "+lastuidstore+" "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "setSales", time.Now().UTC().UnixNano())
		c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		return
	}
	dbm.DbLog("end. конец записи движений в базу "+lastuidstore+" "+time.Now().Format("2006-01-02T15:04:05"), "setSales", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
...
func mkorders(c *gin.Context) {
	...
	stop := c.DefaultQuery("stop", "none")
	start := c.DefaultQuery("start", "none")
	store := dbm.Escape(c.DefaultQuery("store", ""))
	goods := dbm.Escape(c.DefaultQuery("goods", ""))
	...
	slog := dbm.GetLastStateNetwork(3, "makeOrders")
	status := make(map[string]string, 3)
	status["err"] = "no"
	for k, v := range slog {
		switch v[:3] {
...
func getZakaz(c *gin.Context) {
	...
	type OrdersXML struct {
		...
		Orders  []dbm.OrderXML `xml:"orders"`
	}
	dbm.DbLog("beg. выдача заказов за "+period+" "+time.Now().Format("2006-01-02T15:04:05"), "getZakaz", time.Now().UTC().UnixNano())
	z, err := dbm.GetZakazXML(period)
	if err != nil {
		dbm.DbLog("err. ошибка выдачи заказов "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "getZakaz", time.Now().UTC().UnixNano())
...
func setABC(c *gin.Context) {
	...
	dbm.DbLog("beg. начало ABC анализа "+time.Now().Format("2006-01-02T15:04:05"), "setABC", time.Now().UTC().UnixNano())
	err = apiRecalcABC(storeuid, period1, period2)
	if err != nil {
		dbm.DbLog("err. ошибка ABC анализа "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "setABC", time.Now().UTC().UnixNano())
		c.JSON(http.StatusOK, gin.H{"status": "err", "message": errmessage + err.Error()})
		return
	}
	dbm.DbLog("end. конец ABC анализа "+time.Now().Format("2006-01-02T15:04:05"), "setABC", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
}
...
func setsalesmatrix(c *gin.Context) {
	...
	//w["uidStore"] = "="
	//w["uidGoods"] = "="
	dbm.DbLog("beg. начало обновления матрицы товаров "+storeuid+" "+time.Now().Format("2006-01-02T15:04:05"), "setsalesmatrix", time.Now().UTC().UnixNano())
	err := dbm.ReplaceMatrix(matr, w)
	if err != nil {
		dbm.DbLog("err. ошибка обновления матрицы товаров "+storeuid+" "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "setsalesmatrix", time.Now().UTC().UnixNano())
		c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		return
	}
	dbm.DbLog("end. конец обновления матрицы товаров "+storeuid+" "+time.Now().Format("2006-01-02T15:04:05"), "setsalesmatrix", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
}
...
func setbalance(c *gin.Context) {
	...
	dbm.DbLog("beg. начало синхронизации баланса товаров "+storeuid+" "+time.Now().Format("2006-01-02T15:04:05"), "setbalance", time.Now().UTC().UnixNano())
	if len(matrbalance) > 0 {
		err := dbm.UpdateBalance(matrbalance)
		if err != nil {
			dbm.DbLog("err. ошибка синхронизации баланса товаров "+storeuid+" "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "setbalance", time.Now().UTC().UnixNano())
			c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		}
	}
	dbm.DbLog("end. конец синхронизации баланса товаров "+storeuid+" "+time.Now().Format("2006-01-02T15:04:05"), "setbalance", time.Now().UTC().UnixNano())
...
func setstores(c *gin.Context) {
	...
	w := make(map[string]string)
	dbm.DbLog("beg. начало обновления магазинов "+time.Now().Format("2006-01-02T15:04:05"), "setstores", time.Now().UTC().UnixNano())
	err := dbm.InsertTableData("stores", matr, w)
	if err != nil {
		dbm.DbLog("err. ошибка обновления магазинов "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "setstores", time.Now().UTC().UnixNano())
		c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
		return
	}
	dbm.DbLog("end. конец обновления магазинов "+time.Now().Format("2006-01-02T15:04:05"), "setstores", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
}
...
func setordered(c *gin.Context) {
	...
	for _, v := range sm {
		...
		m := make([]map[string]interface{}, 1)
		m[0] = matr
		err = dbm.UpdateTableData("oper", m, cond)
		if err != nil {
			dbm.DbLog("err. ошибка обновления заказов "+err.Error()+" "+time.Now().Format("2006-01-02T15:04:05"), "setordered", time.Now().UTC().UnixNano())
			c.JSON(http.StatusBadRequest, gin.H{"status": "err", "error": true, "message": err.Error()})
			return
		}
	}
	dbm.DbLog("end. конец обновления заказов "+time.Now().Format("2006-01-02T15:04:05"), "setordered", time.Now().UTC().UnixNano())
	c.JSON(http.StatusOK, gin.H{"status": "ok", "message": "ok"})
}
...
func startPage(c *gin.Context) {
	...
	lastdblog := dbm.GetLastStateNetwork(7, "")
...
func tablesPage(c *gin.Context) {
	...
	recs, s, data, err := dbm.GetTable(tabName, pg, gate, "")
	if err != nil {
		hdata["Error"] = err.Error()
	}
	hdata["Error"] = s
...
func salesPage(c *gin.Context) {
	...
	_, _, hdata["Stores"], _ = dbm.GetTable("stores", 0, 0, "tip>=0")
	...
	if len(uidstore) > 0 && len(uidgoods) > 0 {
		datasel, _ := dbm.GetSales(uidstore, uidgoods, per1, per2)
		scnt := 0.0
		ssum := 0.0
		sprof := 0.0
...
func predictPage(c *gin.Context) {
	...
	if len(uidstore) > 0 && len(uidgoods) > 0 {
		...
		cond := "s.uidStore='" + dbm.Escape(uidstore) + "' and s.uidGoods='" + dbm.Escape(uidgoods) + "'"
		stores, err := dbm.GetMagNames(0, uidstore)
		if len(stores) > 0 && stores[0].Tip == 0 { //распределительный склад
			uidstore = "" //продажи и предикт суммируется
			cond = " st.tip>0 and s.uidgoods='" + dbm.Escape(uidgoods) + "'"
			_, lastCenterbalance, _ = dbm.GetLastBalance(stores[0].KeyStore, uidgoods)
		}
		datasel, _ := dbm.GetSales(uidstore, uidgoods, per1, per2, "SMRb") //для перемещений только баланс
		...
		datapredict, _ := dbm.GetPredict(uidstore, uidgoods, per1, per2)
		...
		k, _, matrix, err := dbm.GetTable("salesmatrix", 0, 0, cond)
		if err != nil {
			mx[6] = err.Error()
		}
		switch {
...
func ordersPage(c *gin.Context) {
	...
	_, _, prov, _ := dbm.GetTable("contracts", 0, 0, "s.tip=0")
	...
	hdata["Providers"] = sel
	//массив получателей
	_, _, prov, _ = dbm.GetTable("contracts", 0, 0, "s.tip>=1")
	sel = make([]Hselect, len(prov))
	for k := 1; k < len(prov); k++ {
	...
	recs, zakazs, err := dbm.GetZakaz(numdoc, pg, gate, sortField, sortOrder, filter)
	if err != nil {
		hdata["Error"] = err.Error()
	}
...
func getOrder(c *gin.Context) {
	...
	recs, zakazs, err := dbm.GetZakaz(numdoc, 0, 0, "", "", "")
	if err != nil {
...
func finPage(c *gin.Context) {
	...
	gr := make(map[string]dbm.ProfitGraph)
	//datafin, err := dbm.GetProfitMounth("2017-01-01", time.Now().Format("2006-01-02"))
	if uidstore == "0" {
		uidstore = ""
	}
	if isAuth {
...
func usertab(c *gin.Context) {
	...
	case "GET":
		...
		rows, _, data, err := dbm.GetTable("users", pg-1, gate, cond)
		if err != nil && rows > 0 {
			c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
			return
		}

		Items := make([]dbm.User, 0, len(data))
		//нулевая строка содержит имена полей, пропускаем
		for r := 1; r < len(data); r++ {
			i := dbm.User{}
			v := data[r]
	...
	case "PUT":
		...
		m = append(m, matr)
		err := dbm.UpdateTableData("users", m, cond)
		if err != nil {
			c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
			return
		}
	...
	case "DELETE":
		...
		if len(cond) > 0 {
			err := dbm.DeleteTableData("users", cond)
			if err != nil {
				c.JSON(http.StatusOK, gin.H{"data": "[]", "status": "err", "message": err.Error()})
			}
		}
